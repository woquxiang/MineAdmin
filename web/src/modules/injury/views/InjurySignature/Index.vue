...
import { deleteByIds, page } from '~/injury/api/InjurySignature.ts'
...
defineOptions({ name: 'injury:injury_signature' })
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['injury:injury_signature:create']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['injury:injury_signature:delete']"
          type="danger"
          plain
      ...
      <template #empty>
        <el-empty>
          <el-button
            v-auth="['injury:injury_signature:save']"
            type="primary"
            @click="() => {
              maDialog.setTitle(t('crud.add'))
              maDialog.open({ formType: 'add' })
